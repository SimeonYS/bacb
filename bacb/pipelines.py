...
class BacbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('bacb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bacb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bacb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bacb (date, title, link, content)"
